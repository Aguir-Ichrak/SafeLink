...
function LineChart02({
  ...
  useEffect(() => {
    if (!chart || !legend.current) return;

    const { datasets } = chart.data;
    while (legend.current.firstChild) {
      legend.current.firstChild.remove();
    }

    datasets.slice(0, 2).forEach((dataset, index) => {
